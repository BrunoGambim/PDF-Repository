...
public class FindPDFFilesThatAnUserHasAccessUseCaseTest {
	...
	void initUseCase() {
		...
		when(pdfRepository.findPDFFilesThatCanBeAccessedBy(1L, 1, 1)).thenReturn(new PageAdapter<PDF>(pdfList, 1, 1, 1));
		when(pdfRepository.findPDFFilesThatCanBeAccessedBy(2L, 1, 1)).thenReturn(new PageAdapter<PDF>(pdfList2, 1, 1, 1));
	}
